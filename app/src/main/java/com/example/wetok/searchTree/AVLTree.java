...
public class AVLTree<K extends Comparable, T> implements Serializable {
    ...
    private Node balance(Node node){
        ...
        if (balanceFactor > 1 && getBalanceFactor(node.left) >= 0) {  // L
            return rightRotate(node);
        }
        if (balanceFactor < -1 && getBalanceFactor(node.right) <= 0){ // R
    ...
    private Node insert(Node theRoot, Node y){
        ...
        if(y.key.compareTo(theRoot.key) < 0){ // smaller to left
    ...
    private Node delete(Node node, K key) {
        ...
        if (key.compareTo(node.key) < 0) {
            ...
            // current node is what you want to delete
            ...
            if (node.left == null || node.left.key == null) {
                ...
                // both subtree are non-empty, successor be new node
